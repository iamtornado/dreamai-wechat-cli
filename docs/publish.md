...
执行 `dreamai-wechat-cli publish` 时，CLI 自动完成以下步骤：
...
发布失败、疑似重复草稿时，可打开诊断日志（写到 **stderr**，不影响正常 stdout 上的 HTML/`Media ID` 输出）：
...
调试会打印：**每次运行的 `run` UUID**、各阶段耗时、`contentFingerprint`（标题+原文链接+HTML 的短哈希，用于比对两次运行是否同源）、脱敏后的 `WECHAT_APP_ID` 提示等；并提示 **每次成功的 `publish` 都会调用微信 `draft/add` 新建草稿**，重复执行会产生多篇草稿。

`draft *` 子命令也支持 `--debug` 或同一环境变量。
...
```bash
dreamai-wechat-cli publish -f ./posts/article.md
```
...
```bash
cat article.md | dreamai-wechat-cli publish
```

或：

```bash
echo "# 自动生成的文章" | dreamai-wechat-cli publish
```
...
```bash
dreamai-wechat-cli publish "# Hello Wenyan\n\n这是一篇测试文章"
```
...
* `dreamai-wechat-cli render`：仅渲染 HTML，不发布
* `dreamai-wechat-cli serve`：启动 Server
* `dreamai-wechat-cli theme`：管理主题
